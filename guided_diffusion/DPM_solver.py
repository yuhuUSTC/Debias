...
class DPM_Solver:
    ...
    def dpm_solver_first_update(self, x, x_T, current_step, lamda, s, t, model_s=None, return_intermediate=False):
        """
    ...
    def singlestep_dpm_solver_second_update(self, x, s, t, r1=0.5, model_s=None, return_intermediate=False, solver_type='dpmsolver'):
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            if solver_type == 'dpmsolver':
                ...
                x_t = ((sigma_t / sigma_s) * x  - (alpha_t * phi_1) * model_s + (1. / r1) * (alpha_t * (phi_1 / h + 1.)) * (model_s1 - model_s))
        else:
            phi_11 = torch.expm1(r1 * h)
            phi_1 = torch.expm1(h)
            ...
            if solver_type == 'dpmsolver':
                ...
                x_t = (torch.exp(log_alpha_t - log_alpha_s) * x - (sigma_t * phi_1) * model_s - (1. / r1) * (sigma_t * (phi_1 / h - 1.)) * (model_s1 - model_s))
        if return_intermediate:
    ...
    def multistep_dpm_solver_second_update(self, x, x_T, current_step, lamda, model_prev_list, t_prev_list, t, solver_type="dpmsolver"):
        """
    ...
    def multistep_dpm_solver_third_update(self, x, x_T, lamda, model_prev_list, t_prev_list, t, solver_type='dpmsolver'):
        """
        ...
        if self.algorithm_type == "dpmsolver++":
            ...
            tar = model_prev_0 + D1 + D2
            # self.noise.append(tar)
            # self.x_t.append(x_t)
        return x_t
    ...
    def multistep_dpm_solver_update(self, x, x_T, current_step, lamda, model_prev_list, t_prev_list, t, order, solver_type='dpmsolver'):
        """
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x, x_T, current_step, lamda, t_prev_list[-1], t, model_s=model_prev_list[-1])
        elif order == 2:
            return self.multistep_dpm_solver_second_update(x, x_T, current_step, lamda, model_prev_list, t_prev_list, t, solver_type=solver_type)
        elif order == 3:
            return self.multistep_dpm_solver_third_update(x, x_T, lamda, model_prev_list, t_prev_list, t, solver_type=solver_type)
        else:
            raise ValueError("Solver order must be 1 or 2 or 3, got {}".format(order))
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                x_T = x.clone()
                lamda = torch.linspace(0, 0.15, steps)
                for step in range(1, order):
                    t = timesteps[step]
                    current_step = step
                    x = self.multistep_dpm_solver_update(x, x_T, current_step, lamda[step-1], model_prev_list, t_prev_list, t, step, solver_type=solver_type)
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
                        intermediates.append(x)
                ...
                for step in tqdm(range(order, steps + 1)):
                    ...
                    if lower_order_final and steps < 10:
                        ...
                        step_order = order
                    current_step = step
                    x = self.multistep_dpm_solver_update(x, x_T, current_step, lamda[step-1], model_prev_list, t_prev_list, t, step_order, solver_type=solver_type)
                    if self.correcting_xt_fn is not None:
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:
                        intermediates.append(x)
