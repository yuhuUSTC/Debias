...
def load_data(
    *,
    ...
    if class_cond:
        # Assume classes are the first part of the filename, before an underscore.
        class_names = [bf.basename(path)[0] for path in all_files]
        sorted_classes = {x: i for i, x in enumerate(sorted(set(class_names)))}
        classes = [sorted_classes[x] for x in class_names]
...
def center_crop_arr(pil_image, image_size):
    # We are not on a new enough PIL to support the `reducing_gap` argument, which uses BOX downsampling at powers of two first. Thus, we do it by hand to improve downsample quality.
