...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.sqrt_snr = np.sqrt(self.snr)
        self.sqrt_snr_norm = self.sqrt_snr / min(self.sqrt_snr)
        self.original = self.betas * self.betas / self.alphas / (1.0 - self.alphas_cumprod)
        self.original1 = self.betas / 2 / self.alphas / (1.0 - self.alphas_cumprod)
        self.original2 = self.betas * self.sqrt_alphas_cumprod / np.sqrt(self.alphas) / (1.0 - self.alphas_cumprod)
        self.x0_MinSNR = self.snr.copy()
        self.noise_MinSNR = self.snr.copy()

        for i in range(len(self.snr)):
            if 5/self.snr[i]<1:
                self.noise_MinSNR[i] = 5/self.snr[i]
            else:
                self.noise_MinSNR[i] = 1

        for i in range(len(self.snr)):
            if self.snr[i]<5:
                self.x0_MinSNR[i] = self.snr[i]
            else:
                self.x0_MinSNR[i] = 5
        
        # print(self.alphas_cumprod)
        # print(self.sqrt_snr)
        # print(self.sqrt_snr_norm)
    ...
    def _predict_xstart_from_eps1(self, x_t, t, eps):
        assert x_t.shape == eps.shape
        return (
            x_t - eps
        )
    ...
    def ddim_sample_loop_progressive(
        ...
        """
        Use DDIM to sample from the model and yield intermediate samples from each timestep of DDIM. Same usage as p_sample_loop_progressive().
        """
        if device is None:
            device = next(model.parameters()).device
    ...
    def dpm_solver_uncond_loop(
        ...
        device = next(model.parameters()).device
        x_T = th.randn(*shape, device=device).cuda()
        ...
        with torch.no_grad():
            ...
            dpm_solver = DPM_Solver(model_fn_continuous, noise_schedule, algorithm_type="dpmsolver", correcting_x0_fn="dynamic_thresholding" if thresholding else None,)
    ...
    def UniPC_uncond_loop(
        ...
        from .UniPC import NoiseScheduleVP, model_wrapper, UniPC
        ...
        with torch.no_grad():
            ...
            uni_pc = UniPC(model_fn_continuous, noise_schedule, algorithm_type="data_prediction", correcting_x0_fn="dynamic_thresholding" if thresholding else None,)
            x_sample = uni_pc.sample(
                x_T,
                steps=timestep_respacing,
                skip_type="time_uniform",
                method="multistep",
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            #------------------------original------------------------#
            # noise/x0/v -prediction
            # terms["mse"] = mean_flat((target - model_output) ** 2)


            #------------------------New loss weighting------------------------#
            # noise-prediction —— P2 weighting
            #weight = _extract_into_tensor(1 / (self.p2_k + self.snr)**self.p2_gamma, t, target.shape)
            
            # noise-prediction —— MinSNR weighting
            # weight = _extract_into_tensor(self.noise_MinSNR, t, target.shape)
            
            # noise-prediction —— SNR weighting
            # weight = _extract_into_tensor(1 / self.snr, t, target.shape)
            
            # noise-prediction —— Debias (Ours) weighting
            weight = _extract_into_tensor(1 / self.sqrt_snr, t, target.shape)
